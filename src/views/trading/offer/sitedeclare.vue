		...
		<div class="co-wapper">
			...
			<el-form ref="formDate" :model="formData" label-width="110px" size="small">
				<el-form-item label="模板选择">
					<el-radio v-model="formData.tempOption" v-for="(item,index) in temps" :key="item"  :label="index">{{item}}</el-radio>
				</el-form-item>
				<el-form-item label="申报盘面">
					<el-radio v-model="formData.board" v-for="item in boards" :value="item.value" :key="item.value" :label="item.value">{{item.label}}</el-radio>
				</el-form-item>
					...
					<el-col :span="12">
						...
						<el-form-item label="交货港口" style="display: none">
							<el-select v-model="formData.deliveryPorts" placeholder="请选择">
								<el-option v-for="item in deliveryPorts" :key="item.value" :label="item.label" :value="item"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
					<el-col :span="12">
				...
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="运费差">
							<el-button @click="dialogFormVisible2 = true">填报</el-button>
							&nbsp;&nbsp;基港：青岛
						</el-form-item>
					</el-col>
				</el-row>
...
<script>
	export default {
		...
		methods:{
			getTableData(){
				var _this = this;
				this.$axios.post('data/templateList')
					.then(res => {
